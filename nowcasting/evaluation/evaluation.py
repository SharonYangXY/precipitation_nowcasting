...
def get_hit_miss_counts(prediction, truth, mask=None, thresholds=None, sum_batch=False):
    ...
    thresholds = np.array(thresholds, dtype=np.float32).reshape((1, 1, len(thresholds), 1, 1))
...
def get_rainfall_mse(prediction, truth):
    ret = np.square(prediction - truth).mean(axis=(2, 3))
    ret = ret.sum(axis=1)
    return ret
...
try:
    from nowcasting.evaluation.numba_accelerated import get_GDL_numba, get_hit_miss_counts_numba, \
...
class GPMEvaluation(object):
    ...
    def calculate_stat(self):
        ...
        precision = pod
        recall = a / (a + b)
        f1_score = (2 * precision * recall) / (precision + recall)  # 2a/(2a+c+b)
        # return precision, recall, f1_score
        # return pod, far, csi, hss, gss, mse, mae, gdl
        # return pod, far, csi, hss, gss, mse, mae, balanced_mse, balanced_mae, gdl, pc, recall, f1_score
        return balanced_mse, balanced_mae, pod, far, csi, hss, pc, recall, f1_score
