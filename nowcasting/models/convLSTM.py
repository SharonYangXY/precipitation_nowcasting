    ...
    def forward(self, inputs=None, states=None, seq_len=cfg.RAIN.BENCHMARK.IN_LEN):#seq_len=cfg.RAIN.BENCHMARK.IN_LEN):
        ...
        if states is None:
            c = nn.Parameter(torch.zeros((inputs.size(1), self._num_filter, self._state_height,
                                  self._state_width), dtype=torch.float)).to(cfg.GLOBAL.DEVICE) # added by me.. Pam
            h = nn.Parameter(torch.zeros((inputs.size(1), self._num_filter, self._state_height,
        ...
        for index in range(seq_len):
            # initial inputs
            if inputs is None:
                x = nn.Parameter(torch.zeros((h.size(0), self._input_channel, self._state_height,
                                      self._state_width), dtype=torch.float)).to(cfg.GLOBAL.DEVICE)
            else:
                x = inputs[index, ...]
