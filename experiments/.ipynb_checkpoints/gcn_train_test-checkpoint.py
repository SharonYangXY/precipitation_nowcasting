...
def train_and_test(train_dataset, test_dataset, val_dataset, encoder_forecaster, optimizer, criterion, lr_scheduler,
    ...
    for cur_epoch in tqdm(range(epochs)):
        ...
        plot_result(writer, cur_epoch,
                    (train_pod, train_far, train_csi, train_hss, train_gss, train_mse, train_mae, train_balanced_mse,
                     train_balanced_mae, train_gdl),
                    (valid_pod, valid_far, valid_csi, valid_hss, valid_gss, valid_mse, valid_mae, valid_balanced_mse,
                     valid_balanced_mae, valid_gdl))
...
def plot_result(writer, itera, train_result, valid_result):
    train_pod, train_far, train_csi, train_hss, train_gss, train_mse, train_mae, train_balanced_mse, train_balanced_mae, train_gdl = train_result
    train_pod, train_far, train_csi, train_hss, train_gss, train_mse, train_mae, train_balanced_mse, train_balanced_mae, train_gdl = \
        np.nan_to_num(train_pod), \
        np.nan_to_num(train_far), \
        np.nan_to_num(train_csi), \
        np.nan_to_num(train_hss), \
        np.nan_to_num(train_gss), \
        np.nan_to_num(train_mse), \
        np.nan_to_num(train_mae), \
        np.nan_to_num(train_balanced_mse), \
        np.nan_to_num(train_balanced_mae), \
        np.nan_to_num(train_gdl)

    valid_pod, valid_far, valid_csi, valid_hss, valid_gss, valid_mse, valid_mae, valid_balanced_mse, valid_balanced_mae, valid_gdl = valid_result
    valid_pod, valid_far, valid_csi, valid_hss, valid_gss, valid_mse, valid_mae, valid_balanced_mse, valid_balanced_mae, valid_gdl = \
        np.nan_to_num(valid_pod), \
        np.nan_to_num(valid_far), \
        np.nan_to_num(valid_csi), \
        np.nan_to_num(valid_hss), \
        np.nan_to_num(valid_gss), \
        np.nan_to_num(valid_mse), \
        np.nan_to_num(valid_mae), \
        np.nan_to_num(valid_balanced_mse), \
        np.nan_to_num(valid_balanced_mae), \
        np.nan_to_num(valid_gdl)

    for i, thresh in enumerate(cfg.HKO.EVALUATION.THRESHOLDS):
        writer.add_scalars("pod/{}".format(thresh), {
    ...
    for i, thresh in enumerate(cfg.HKO.EVALUATION.THRESHOLDS):
        writer.add_scalars("gss/{}".format(thresh), {
            "train": train_gss[:, i].mean(),
            "valid": valid_gss[:, i].mean()
        }, itera)
        for j in range(train_gss.shape[0]):
            writer.add_scalars("gss/{}".format(thresh), {
                "train_frame" + str(j + 1): train_gss[j, i],
                "valid_frame" + str(j + 1): valid_gss[j, i]
            }, itera)
    ...
    writer.add_scalars("gdl", {
        "train": train_gdl.mean(),
        "valid": valid_gdl.mean(),
        "train_last_frame": train_gdl[-1],
        "valid_last_frame": valid_gdl[-1],
    }, itera)
